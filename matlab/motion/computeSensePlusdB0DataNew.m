...
function [obj]  = computeSensePlusdB0DataNew( x, img, in_str, kspace_data, sens, mask_nav)
    ...
    phs_B0 = exp(1i*2*pi*reshape(sum(Bas.*x(1,1:nb),2),size_nav).*reshape(TEs,1,1,1,1,[])); % B0 induced phase, size_nav+1+T, 1 is to allow sensitivity
