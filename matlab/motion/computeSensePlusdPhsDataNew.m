...
function [obj]  = computeSensePlusdPhsDataNew( x, img, in_str, kspace_data, sens, mask_nav)
    ...
    phs_bg0 = exp(1i*reshape(sum(Bas.*x(1,1:9),2),size_nav) );
    phs_B0 = exp(1i*2*pi*reshape(sum(Bas.*bcoef,2),size_nav).*reshape(TEs,1,1,1,1,[])); % B0 induced phase, size_nav+1+T, 1 is to allow sensitivity
    img_tmp = img .* dphs .* phs_B0 .* phs_bg0;
    % perform translation and rotation
    [~,~,~,img_warp] = motion_ops(size_nav, mot_params,0,img_tmp);
